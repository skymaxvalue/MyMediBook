...
export class MedicineOrdersComponent implements OnInit {
  ...
  async downloadPrescriptionPdf(): Promise<void> {
    ...
    try {
      ...
      console.error("Error while generating prescription PDF:", error);
    }
  }
  ...
  get readyOrders(): number {
    return this.orders.filter((x) => x.orderStatus === "Ready").length;
  }

  get transitOrders(): number {
    return this.orders.filter((x) => x.orderStatus === "In Transit").length;
  }
  ...
  applyFilters(): void {
    let data: MedicineOrder[] = [...this.orders];
    ...
    if (value) {
      data = data.filter(
        (order) =>
          order.patientName?.toLowerCase().includes(value) ||
          order.drugName?.toLowerCase().includes(value) ||
          order.doctorName?.toLowerCase().includes(value) ||
    ...
    switch (this.sortValue) {
      ...
      // Name Z-A
      case "nameDesc":
        data.sort((a, b) => b.patientName.localeCompare(a.patientName));

        break;

      // Ready
      case "ready":
        data = data.filter((x) => x.orderStatus === "Ready");

        break;

      // In Transit
      case "transit":
        data = data.filter((x) => x.orderStatus === "In Transit");
  ...
  requestRefill(order: MedicineOrder): void {
    this.toast.info("Info", `Refill request submitted for ${order.drugName}`);
  }
  ...
  downloadPDF(): void {
    ...
    html2canvas(data, {
      ...
      pdf.save(`medicine-order-${this.selectedOrder?.orderId}.pdf`);
    });
  }
