...
export class RescheduleComponent implements OnInit {
  ...
  ngOnInit() {
    ...
    if (history.state) {
      ...
      this.store.select(selectGetTimeSlotOfDoctor).subscribe((res: any) => {
        if (res?.data) {
  ...
  async confirmReschedule() {
    ...
    this.appointment.date = this.formatDate(this.form.value.appointmentDate);
    ...
    this.appointment.otherReason = this.form.value.otherReason;

    localStorage.setItem("appointmentToReschedule", JSON.stringify(this.appointment));
    const payload = {
      ...
      lastUpdatedBy:
        this.logdinUser.firstName +
        " " +
        this.logdinUser.middleName +
        " " +
        this.logdinUser.lastName,
    ...
    await this.store.select(selectRescheduledAppointment).subscribe((res: any) => {
      if (res) {
        ...
        this.router.navigate(["/patient/dashboard/appointment-reschedule-successfull"], {
          // state: {
          //   appointment
          // }
        });
  ...
  disablePastSlots() {
    ...
    this.timeSlots.forEach((slot: any) => {
      ...
      slot.disabled = slot.booked || !slot.isAvailable || slotDate <= now;
    });
  }
