...
export class MessagesComponent implements OnInit {
  ...
  filteredMessages = computed(() => {
    ...
    if (filter !== "all") {
      result = result.filter((message) => message.type === filter);
    }
   ...
   * All message count
   */
  allCount = computed(() => this.messages().filter((message) => !message.archived).length);

  /**
   * Archived message count
   */
  archiveCount = computed(() => this.messages().filter((message) => message.archived).length);
  ...
  totalPages = computed(() => {
    const total = Math.ceil(this.filteredMessages().length / this.pageSize());
