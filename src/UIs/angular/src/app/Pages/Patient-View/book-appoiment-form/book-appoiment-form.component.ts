...
import {
  selectGetProfileDataByProfileId,
  selectGetProfileListByPatientId,
} from "src/app/Store/Patient/patient.selectors";
...
export class BookAppoimentFormComponent implements OnInit {
  ...
  InitialApiCall() {
    ...
    this.store.select(selectAppointmentStatusList).subscribe((res: any) => {
      const visitTypeData = res.find((item: any) => item.category === "VisitType");

      const visitPurposeData = res.find((item: any) => item.category === "VisitPurpose");
  ...
  // }
  async onProfileChange(event: any) {
    this.store.dispatch(getProfileDataByProfileId({ profileId: event.profileId }));
  ...
  async submitForm() {
    ...
    if (this.bookingForm.invalid) {
      ...
      await this.store.select(selectRequestedOTP).subscribe((res: any) => {
        if (res) {
          const otpDeviceDetails: any = {
            otpDevice: this.bookingForm.value.otpMethod,
            value:
              this.bookingForm.get("otpMethod")?.value === "mobile"
