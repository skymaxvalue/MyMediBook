...
import {
  ...
  selectMyAppointmentList,
} from "src/app/Store/Appointments/appointment.selcetors";
...
export class MyAppointmentComponent implements OnInit {
  ...
  ngOnInit() {
    ...
    this.store.select(selectMyAppointmentList).subscribe((res: any) => {
  ...
  filteredPatients = computed(() => {
    ...
    return this.relativeList().filter(
      (patient) =>
  ...
  filteredAppointments = computed(() => {
    ...
    if (this.selectedStatus()) {
      data = data.filter(
        (x) => (x.appointmentStatus || "").toLowerCase() === this.selectedStatus().toLowerCase()
      );
    }
  ...
  rescheduleAppointment(appointment: any) {
    const formattedDate = formatDate(appointment.appointmentDate, "dd MMM yyyy", "en-US");
    const modalData: any = {
      type: "reschedule",
      ...
      confirmTitle: "Reschedule not allowed.",

      confirmText: "This appointment has already started. Rescheduling is not allowed.",
...
`;
  ...
  <strong>${appointment.slotStartTime}</strong>, which is now less than 24 hours away. Rescheduling at this time may result in limited availability and could be subject to our late‑change policy.`;
      ...
      this.confirmationService.open({
        ...
        confirmTitle: "Are you sure you want to reschedule this appointment?",
    ...
    this.confirmationService.response$.pipe(take(1)).subscribe((confirmed) => {
      if (!confirmed) {
        return;
      }

      this.router.navigate(["/patient/dashboard/appointment-reschedule"], {
        state: {
          appointment,
        },
      });
  ...
  async cancelAppointment(appointment: any) {
    ...
    // Appointment already started
    if (diffInHours <= 0) {
      modalData.confirmText = "This appointment has already started. Cancellation is not allowed.";
 ...
 , which is now less than 24 hours away. You can still cancel, but please note that last-minute cancellations may affect availability for other patients. 
  ...
  <strong>Would you like to continue with cancellation?</strong> `;
    ...
    // More than 24 Hours
      ...
      modalData.message =
        "Your appointment is more than 24 hours away. You can still cancel without any charges.  <br><br> <strong>Would you like to continue with cancellation?</strong>";
      modalData.displayRules = [cancelRules[1]];
      modalData.confirmButton = "Yes, Cancel Appointment";
      modalData.cancelButton = "Keep Appointment";
    }
    ...
    this.confirmationService.response$.pipe(take(1)).subscribe(async (confirmed) => {
