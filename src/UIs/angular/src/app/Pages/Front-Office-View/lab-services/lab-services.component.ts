...
export class LabServiceComponent implements OnInit {
  ...
  private loadPatients(): Patient[] {
    ...
    if (!raw) {
      ...
      localStorage.setItem(this.PATIENTS_KEY, JSON.stringify(seeded));
    ...
    try {
      ...
      return Array.isArray(parsed) ? parsed : this.seedPatients();
    } catch {
  ...
  searchPatients(): void {
    const ln = this.patientLastNameSearch.trim().toLowerCase();

    const dob = this.patientDobSearch;

    const phone = this.normalizePhone(this.patientMobileSearch);
    ...
    this.patientSearchResults = this.allPatients.filter((patient) => {
      const lastDobMatch =
        ln !== "" &&
        patient.lastName.toLowerCase().includes(ln) &&
        (dob === "" || patient.dob === dob);

      const phoneMatch = phone !== "" && this.normalizePhone(patient.phone).includes(phone);
  ...
  selectPatient(patient: Patient): void {
    ...
    this.patientLastNameSearch = patient.lastName;

    this.patientDobSearch = patient.dob;

    this.patientMobileSearch = patient.phone;
    ...
    this.patientInsurance = patient.insurance || "";

    this.patientMemberId = patient.memberId || "";
  ...
  getSelectedTests(): string[] {
    return this.tests.filter((test) => test.selected).map((test) => test.name);
  }
  ...
  getSelectedSpecimenLabel(): string {
    const specimen = this.specimenTypes.find((s) => s.value === this.selectedSpecimen);

    return specimen ? specimen.label : "";
  }
  ...
  openSelectPatientModal(): void {
    ...
    this.modalPatientResults = [...this.allPatients];
  ...
  searchModalPatients(): void {
    const search = this.modalPatientSearch.trim().toLowerCase();

    const dob = this.modalPatientDob;

    this.modalPatientResults = this.allPatients.filter((patient) => {
      const fullName = `${patient.firstName} ${patient.lastName}`.toLowerCase();

      const nameMatch = !search || fullName.includes(search);

      const dobMatch = !dob || patient.dob === dob;
  ...
  confirmModalPatient(): void {
    ...
    this.selectPatient(this.modalSelectedPatient);
  ...
  saveDraft(): void {
    ...
    this.draftId = this.generateSequentialId("DFT", "labServiceDraftCounter_v1");
    ...
    const draft: LabDraft = {
      ...
      specimen: this.getSelectedSpecimenLabel(),

      tests: this.getSelectedTests(),

      priority: this.getSelectedPriorityLabel(),

      providerName: this.providerName,

      npi: this.providerLocation,

      collectionDateTime: this.collectionDateTime,
    ...
    this.appendToStoredList(this.DRAFTS_KEY, draft);
  ...
  confirmRequest(): void {
    this.requestId = this.generateSequentialId("LAB", "labServiceRequestCounter_v1");
    ...
    this.appendToStoredList(this.REQUESTS_KEY, request);
  ...
  resetForm(): void {
    ...
    this.resultDelivery = "Print / Pickup";
  ...
  private generateSequentialId(prefix: string, counterKey: string): string {
  ...
  private appendToStoredList(storageKey: string, entry: any): void {
