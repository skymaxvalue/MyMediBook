...
import {
  createAssociatesSchedule,
  getAllAssociates,
  getRoleDepaSpecia,
  getWeekDays,
} from "src/app/Store/Doctor/doctor.action";
import {
  selectCreatedAssociateSchedule,
  selectGelAllAssociate,
...
export class AssignScheduleAsspciationComponent implements OnInit {
  ...
  onRoleChange(roleId: number) {
    const selectedRole = this.allRoles.find((x: any) => x.roleId == roleId);
  ...
  updateNameOptions(): void {
    const department = this.scheduleForm.get("departmentId")?.value;

    this.filteredAssociates = this.associateRecords.filter(
      (x) => !department || x.department === department
    );
  ...
  updateSpeciality(event: any): void {
    const record = this.associateRecords.find((x) => x.associateId == event.target.value);
  ...
  findSelectedRecord() {
    ...
    return this.associateRecords.find((x) => x.associateId == form.associateId);
  }
  ...
  validateCurrentStep(): boolean {
    if (this.currentStep === 0) {
      const controls = ["associateId", "roleId", "departmentId"];
      ...
      return controls.every((control) => this.scheduleForm.get(control)?.valid);
    }

    if (this.currentStep === 1) {
      const controls = ["fromDate", "toDate", "fromTime", "toTime"];

      controls.forEach((control) => this.scheduleForm.get(control)?.markAsTouched());
  ...
  populateSummary(): void {
    ...
    const associate = this.associateRecords.find((x) => x.associateId == form.associateId);

    const department = this.departments.find((x) => x.departmentId == form.departmentId);

    const role = this.allRoles.find((x: any) => x.roleId == form.roleId);

    const designation = this.designations.find((x) => x.designationId == form.designationId);

    const speciality = this.speciality.find((x) => x.specialityId == form.specialityId);
