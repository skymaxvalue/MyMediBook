...
export class HelthCareAssociationComponent {
  ...
  stepStatus: ("pending" | "active" | "completed")[] = ["active", "pending", "pending", "pending"];
  ...
  get personalInfoGroup(): FormGroup {
    return this.associateForm.get('personalInfo') as FormGroup;
  }

  get qualificationGroup(): FormGroup {
    return this.associateForm.get('qualification') as FormGroup;
  }

  get experienceGroup(): FormGroup {
    return this.associateForm.get('experience') as FormGroup;
  }

  get employmentDetailsGroup(): FormGroup {
    return this.associateForm.get('employmentDetails') as FormGroup;
  }
  ...
  async onSelectCountry(countriId: number) {
    await this.store.dispatch(AuthActions.getStates({ countryId: countriId }));
    this.store
  ...
  async onSelectState(stateId: number) {
    await this.store.dispatch(AuthActions.getCities({ stateId: stateId }));
    this.store
  ...
  getStepStatus(step: number): string {
    ...
    this.stepStatus[step] =
      this.currentStep === step ? "active" : forms[step]?.valid ? "completed" : "pending";
  ...
  async submit() {
    ...
    const payload = {
      ...
      createdBy:
        this.associateForm.value.personalInfo.firstName +
        " " +
        this.associateForm.value.personalInfo.lastName,
    ...
    await this.store.select(selectRegisterAssociate).subscribe((res: any) => {
      if (res) {
        if (res.isSuccess === 1) {
